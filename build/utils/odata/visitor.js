...
class PGVisitor extends visitor_1.Visitor {
    ...
    VisitLiteral(node, context) {
        if (this.options.useParameters) {
            const value = odata_v4_literal_1.Literal.convert(node.value, node.raw);
    ...
    VisitArrayOrObject(node, context) {
        if (this.options.useParameters) {
            const value = node.value.value.items.map((item) => (item.value === "number" ? parseInt(item.raw, 10) : item.raw));
            context.literal = value;
            this.parameters.push(value);
            this.where += `:${this.parameters.length - 1}`;
        }
    ...
    VisitMethodCallExpression(node, context) {
        ...
        switch (method) {
            case "contains":
                ...
                if (this.options.useParameters) {
                    const value = odata_v4_literal_1.Literal.convert(params[1].value, params[1].raw);
                    this.parameters.push(`${value}`);
                    this.where += ` ~* :${this.parameters.length - 1}`;
                }
                else
            ...
            case "substringof":
                ...
                if (params[0].value == "Edm.String") {
                    if (this.options.useParameters) {
                        const value = odata_v4_literal_1.Literal.convert(params[0].value, params[0].raw);
                        this.parameters.push(`%${value}%`);
                        this.where += ` ILIKE :${this.parameters.length - 1}`;
                    }
                    else
